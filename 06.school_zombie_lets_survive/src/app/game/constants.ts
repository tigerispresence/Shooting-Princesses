...
/**
 * 파티 폭죽 (스테이지 4부터) — 분필·알람이 "끌어오기"라면 폭죽은 "밀어내기".
 * 던지면 착지 지점에서 「펑!」 터지고 반경 안 좀비가 반대쪽으로 도망친 뒤 어리둥절한다.
 */
export const POPPER = {
  speed: 240,
  range: 200,
  ...
  /** 도망치는 시간 */
  fleeMs: 1800,
  /** 도망친 뒤 별이 뱅뱅 도는 시간 (그동안 못 움직이고 못 본다) */
  dizzyMs: 2000,
  /** 도망 목표 지점까지의 거리 */
  fleeDist: 170,
  max: 3,
...
/**
 * 바나나 껍질 (스테이지 3부터) — 발밑에 놓는 함정. 좀비가 밟으면 「슈욱— 철푸덕!」
 * 교장 선생님도 밟으면 넘어져 연설 준비가 끊긴다. 플레이어와 친구는 안 미끄러진다.
 */
export const BANANA = {
  /** 밟았다고 치는 거리 */
  stepR: 18,
  /** 좀비가 누워 있는 시간 */
  slipMs: 3500,
  /** 교장 선생님이 누워 있는 시간 (이후 회복 2.2초가 그대로 이어진다) */
  bossSlipMs: 1500,
  /** 놓은 직후 이만큼은 안 터진다 (놓자마자 옆 좀비가 밟아 버리는 걸 막는 용도가 아니라, 놓는 연출 시간) */
  armMs: 300,
  max: 3,
};
